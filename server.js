...
http.listen(3000, function () {
    ...
    mongoClient.connect("mongodb://localhost:27017", function (error, client) {
        var database = client.db("s_network")
        console.log("Database connected");

    app.get("/profileViews", (request, result) => {
    ...
    app.get("/post/:id", (request,result) => {
    ...
    app.post("/addPost", (request, result) => {
        ...
        database.collection("users").findOne({
            ...
            if (user == null) {
                ...
                if (request.files.image.size > 0 && request.files.image.type.includes("image")) {
                    ...
                    // Read the file
                    fileSystem.readFile(request.files.image.path, (err, data) => {
                        if (err) throw err;
                        console.log('File read!');

                        // Write the file
                        fileSystem.writeFile(image, data, (err) => {
                            if (err) throw err;
                            console.log('File written!');
                        });

                        // Delete the file
                        fileSystem.unlink(request.files.image.path, (err) => {
                            if (err) throw err;
                            console.log('File deleted!');
                        });
                    });
                ...
                if (request.files.video.size > 0 && request.files.video.type.includes("video")) {
                    ...
                    // Read the file
                    fileSystem.readFile(request.files.video.path, (err, data) => {
                        if (err) throw err;
                        console.log('File read!');

                        // Write the file
                        fileSystem.writeFile(video, data, (err) => {
                            if (err) throw err;
                            console.log('File written!');
                        });

                        // Delete the file
                        fileSystem.unlink(request.files.video.path, (err) =>  {
                            if (err) throw err;
                            console.log('File deleted!');
                        });
                    });
    ...
    app.post("/getNewsfeed", (request, result) => {
        var accessToken = request.fields.accessToken;
        database.collection("users").findOne({
            "accessToken": accessToken
        }, (error, user) => {
    ...
    app.get("/page/:id", function(request, result){
    ...
    app.post("/toggleLikePage", function(request, result){
        ...
        database.collection("users").findOne({
            ...
            if (user == null) {
                result.json({
                    "status": "error",
                    "message": "User has been logged out. try login again"
                });
            } else {
                database.collection("pages").findOne({
                    "_id": ObjectId(_id)
